...
// Payments remain out of scope. Add collection/retention clauses in a new version when they ship.
...
handles personal data of users of ChartDraw (the "Service").
...
② **The five questions provided each day, and the pattern book's list and descriptions,
are available without an account.** If you use the Service without creating an account,
the Company collects no personal data whatsoever, and your learning records are stored
solely on your device.

③ All questions, the case questions within the pattern book, keeping learning records in
your account, and the Daily Ranking require signing in. Personal data is collected **only
when you create an account**. The Service contains no payment functionality.

④ When you create an account or sign in, **the learning records previously stored on your
device are transferred to and merged into that account's records on our servers.**
...
- Learning records: quiz results, number of attempts, scores, accuracy rate, participation
  dates
- Daily Ranking records: participation date, number of attempts per question, calculated
  score, submission time
- **Display Name**: a name generated automatically by the Company to identify a Member on
...
- During sign-in and authentication, your IP address, access time, and authentication event
  records are stored by our authentication infrastructure. These are used solely to detect
  unauthorised access and respond to security incidents, and are destroyed when you withdraw.
...
| Calculating and displaying the Daily Ranking | Daily Ranking records, Display Name |
| Detecting and restricting improper ranking participation | Daily Ranking records, social account identifier |
...
## Article 3-2 (Information Visible to Other Users)

① When a Member submits a Daily Ranking entry, **their Display Name, that day's score, and
rank become visible to other users.** An entry is created only when the Member actively
submits it within the Service; signing in alone does not create one.

② Only the Display Name, score, and rank are disclosed. Email addresses, social account
identifiers, and internal account identifiers are never provided to other users under any
circumstances.

③ The Display Name is generated automatically from a fixed word list defined by the
Company and contains no personal data such as a real name or email address. Because users
cannot enter arbitrary text, the Service contains no user-written posts or phrases.

④ Rankings may be viewed only for dates within the past 30 days.

⑤ Deleting your account also deletes your ranking records and Display Name, after which
they are no longer shown to other users.
...
① Except as described in Paragraph 2, and except where required by lawful process from
investigative or other competent authorities, the Company does not provide users' personal
data to third parties.

② Where a Member actively submits a Daily Ranking entry, their Display Name, score, and rank
become visible to other users of the Service in accordance with Article 3-2. **This
disclosure occurs at the moment the Member submits the entry within the Service, together
with notice; signing in alone does not disclose anything.** Declining to enter the ranking
does not restrict use of the learning features.
...
records and Daily Ranking records stored on the server, and they cannot be recovered.
...
⑤ A Daily Ranking entry is submitted once per day and its score cannot be modified.
However, you may at any time request that the entry be **withdrawn from display and
deleted** from [Settings] within the Service. Deleting your account also removes it.

## Article 9 (Children Under 14)

The Company does not collect personal data from children under the age of 14, and **users
under 14 cannot create an account.** Children under 14 may still use the five questions
provided each day without an account, in which case no personal data is collected and they
do not take part in the Daily Ranking.
...
- Server-side fixing of the fields returned when rankings are queried, so that other users'
  account identifiers are never included in the response
